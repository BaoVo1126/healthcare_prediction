...
class AutoPreprocessor:
    ...
    def __init__(self, target_col, drop_cols=None,
        ...
        self.val_size     = val_size    # 0.0 = không dùng val set
        self.test_size    = test_size
        self.random_state = random_state

        # Lưu lại sau khi fit — dùng để transform data mới
        self._le_dict     = {}          # {col: LabelEncoder} cho từng cột categorical
        self._le_target   = LabelEncoder()
        self._scaler      = StandardScaler()
        self._feature_names = None
        self._cat_cols    = []

    # ── PUBLIC: fit + transform (dùng khi có labeled data) ─
    def fit_transform(self, df):
        ...
        # Bước 2: Fix giá trị âm trong Billing Amount
        df = self._fix_negatives(df)

        # Bước 3: Feature Engineering từ datetime
        ...
        # Bước 5: Split
        #  Quan trọng: split TRƯỚC khi fit encoder/scaler
        #    → scaler chỉ được "nhìn thấy" train data
        if self.val_size > 0:
            # Bước 5a: tách test ra trước
            # Ví dụ: val=0.1, test=0.1 → test_ratio = 0.1 / 1.0 = 0.1
            ...
            # Bước 5b: tách val từ phần còn lại (temp)
            # val_ratio_of_temp = val / (1 - test) = 0.1 / 0.9 ≈ 0.111
        ...
        # Bước 6: Encode categorical — fit CHỈ trên train
        self._cat_cols = X_train.select_dtypes(include='object').columns.tolist()
        for col in self._cat_cols:
            le = LabelEncoder()
            X_train[col] = le.fit_transform(X_train[col].astype(str))
            # transform val/test: giá trị chưa gặp → map về class đầu tiên
        ...
        # Bước 8: Scale — fit CHỈ trên train
    ...
    # ── PUBLIC: transform data mới (không fit lại) ─────────
